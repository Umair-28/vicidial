...
class CrmLeadTarget(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users", string="Salesperson", required=True, ondelete="cascade"
    )
    ...
    achieved_leads = fields.Integer(
        string="Achieved Leads", compute="_compute_achieved_leads", store=False
    )
    won_lead_ids = fields.Many2many(
        "crm.lead", string="Won Leads", compute="_compute_won_lead_ids", store=False
    )
    achievement_percentage = fields.Float(
        string="Achievement %", compute="_compute_achievement_percentage", store=False
    )
    ...
    def _compute_name(self):
        for record in self:
            if record.user_id and record.period_type:
                period_name = dict(record._fields["period_type"].selection).get(
                    record.period_type
                )
        ...
        for record in self:
            ...
            if won_stage:
                ...
                for lead in won_leads:
                    check_date = (
                        lead.date_closed if lead.date_closed else lead.write_date
                    )
                    if check_date:
        ...
        for record in self:
            if record.target_leads > 0:
                record.achievement_percentage = (
                    record.achieved_leads / record.target_leads
                ) * 100
            else:
    ...
    def _compute_won_lead_ids(self):
        for record in self:
            ...
            filtered = leads.filtered(
                lambda l: (l.date_closed or l.write_date).date() >= record.date_start
                and (l.date_closed or l.write_date).date() <= record.date_end
            )
