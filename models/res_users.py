...
class ResUsers(models.Model):
    ...
    lead_target_ids = fields.One2many(
        "crm.lead.target", "user_id", string="Lead Targets"
    )
    lead_target_count = fields.Integer(
        string="Number of Targets", compute="_compute_lead_target_count"
    )
    current_lead_target_id = fields.Many2one(
        "crm.lead.target",
        string="Current Active Target",
        compute="_compute_current_target",
        store=False,
    )
    current_target_leads = fields.Integer(
        string="Current Target", related="current_lead_target_id.target_leads"
    )
    current_achieved_leads = fields.Integer(
        string="Current Achieved", related="current_lead_target_id.achieved_leads"
    )
    current_achievement_percentage = fields.Float(
        string="Current Achievement %",
        related="current_lead_target_id.achievement_percentage",
    )
    auto_create_lead_target = fields.Boolean(
        string="Auto Create Lead Target",
        default=False,
        help="Automatically create monthly lead target for this user",
    )
    default_lead_target = fields.Integer(
        string="Default Lead Target",
        default=10,
    ...
    @api.depends(
        "lead_target_ids",
        "lead_target_ids.state",
        "lead_target_ids.date_start",
        "lead_target_ids.date_end",
    )
    ...
    def _check_ip_restriction(self):
        ...
        if not request:
            _logger.warning(
                "No request object available for IP check for user %s", self.login
            )
            return True
        ...
        if not is_allowed:
            _logger.warning(
                ...
                self.id,
                client_ip,
                ", ".join(allowed_ips_list),
            )
            raise AccessDenied(
                _(
                    "Access denied: Your IP address (%s) is not authorized to access this account. "
        ...
        _logger.info(
            'User "%s" logged in successfully from IP %s', self.login, client_ip
        )
        return True
        ...
        try:
            ...
            for allowed_ip in allowed_ips_list:
                try:
                    ...
                    _logger.warning(
                        "Invalid IP/Network format: %s - %s", allowed_ip, str(e)
                    )
                    continue
