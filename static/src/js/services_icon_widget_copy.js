...
function injectServicesWidget() {
  ...
  Array.from(select.options).forEach((opt) => {
    ...
    btn.innerHTML = `
    <div class="icon-container">${getIcon(cleanValue)}</div>
    <div class="label-container">
        ${formatServiceLabel(opt.text)}
    </div>
`;
...
function formatServiceLabel(text) {
  ...
  if (!match) {
    // No brackets → return text normally
    return `<span class="service-title">${text}</span>`;
  }

  const name = match[1].trim(); // e.g., Electricity & Gas
  const type = match[2].trim(); // e.g., Website
...
function removeSpaces() {
    ...
    if (url.includes("crm")) {
        let elements = document.getElementsByClassName("oe_title");
